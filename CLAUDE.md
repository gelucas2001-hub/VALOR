...
|---|---|---|
| `actualizar.py` | Baja datos de ESPN y calcula λ. Corre solo, 2 veces por día, vía GitHub Actions. **Ocho competiciones**: `arg.1`, `bra.1`, `eng.1`, `fra.1`, `esp.1`, `arg.copa` y las dos de CONMEBOL. **`jpn.1` entró y salió el mismo día (2026-09-02)**: entró por tener el mejor punto estimado de ROI de las seis medidas y salió porque Bet365 no la cotiza. La reemplazó **`esp.1`**, que NO entra por su ROI (−5.33% ±8.52, ruido) sino por el mercado: es la escalera de props de jugador más profunda que ofrece Bet365, y los props son el único hilo con señal medida del proyecto (§22bis). Ver TRASPASO §24 y el bloque de COMPETICIONES en el propio archivo, que es la fuente de verdad | Con cuidado — es el motor |
...
| `test_registro.js`, `test_alineacion.js`, `test_ejes.js`, **`test_probabilidad.js`** | Suites que leen el `index.html` publicado, no una copia. **Son cuatro, no dos**: el 2026-09-02 se invirtió la dirección de la serie del jugador y `test_probabilidad.js` quedó roto una hora porque no figuraba acá y nadie lo corrió. Si tocás la app, corré las cuatro | Correr después de tocar la app |
| `verificar_app.js` | La revisión interna de `index.html`: clases de CSS sin uso, funciones muertas, colores fuera de los tokens, y las regiones marcadas que leen los tests. **No figuraba en esta tabla hasta el 2026-09-03**, y el mismo día se le arreglaron tres puntos ciegos: exentaba a toda función mencionada en cualquier suite (así se escondió `senalDividida()`, sin llamador y con tres tests en verde), no miraba declaraciones repetidas (`fechaCorta()` estuvo declarada dos veces dos días), y volcaba 80 clases en una lista sin orden. Ver TRASPASO §28. `node verificar_app.js` | Correr después de tocar la app |
...
| `historico.py` | Baja y normaliza el historial largo de football-data.co.uk: 6310 partidos de arg, 5544 de bra, **4180 de eng y 3857 de fra** (agregadas el 2026-08-25), con cuota de cierre de Pinnacle. Es la base de las mediciones serias. Lee los **dos** formatos de la fuente — un archivo único para las ligas nuevas, uno por temporada para las clásicas de Europa. Solo el segundo trae estadísticas por partido, y esa es la razón por la que en Inglaterra se distinguen los equipos y en Argentina no **Desde el 2026-09-02 avisa de qué casa sale la cuota**: football-data dejó de llenar la de cierre de Pinnacle a mitad de la 2025/26 (la columna existe y viene vacía), y la cadena de respaldo Pinnacle→promedio→Bet365 lo tapaba porque funciona bien. El total no lo muestra —España tiene 95% de Pinnacle— pero desde 2026 las ocho ligas van de 0% a 12%. `cobertura_fuente()` separa el total de la ventana reciente y levanta alerta. Ver TRASPASO §26 | A mano |
...
| `medir_props.py` | El eje Jugadores contra **PLATA**, al precio real de Bet365 — la primera vez que se mide dinero acá, cuando `medir_jugadores.py` solo medía calibración. Usa la escalera de precios entera y las varias fotos por partido que `props_jugadores.json` venía acumulando. Cruza nombres **solo por igualdad exacta**: los que no cruzan son en su mayoría otros jugadores (Clever Ferreira contra Pablo Ferreira), y un cruce difuso los funde. Trae su propia vara: la **deriva** de los escalones que NO se apuestan, sin la cual un CLV positivo puede ser la casa achicando el margen sobre la hora. Trae ademas dos controles que el error estandar no da: **dejar un partido afuera por vez** (las apuestas de un mismo partido no son independientes) y **cuantas lineas quedaron clavadas** (ahi el CLV es cero por construccion). **El CLV se mide en puntos de probabilidad implicita (`1/cierre - 1/precio`), no en el cociente `precio/cierre - 1`**: el cociente es asimetrico —sin techo arriba, acotado en -100% abajo— y el 2026-09-02 una sola linea que fue de 13.00 a 3.50 movio el promedio de +3.32% a +6.11% y cuadruplico el error estandar, bajando la significancia de 2.6 a 1.5 e.e. sin que la señal se hubiera movido. Las dos escalas se siguen imprimiendo; manda `pp`. **Medido asi sobre 26 partidos y 96 apuestas: CLV +1.13 pp ±0.37, y contra la deriva (-0.02 pp ±0.07) elegimos mejor por +1.15 pp ±0.38, 3.0 errores estandar**; dejando un partido afuera por vez el peor caso queda en +0.74 pp, o sea que ninguno lo carga. Ojo: la deriva en escala simetrica es CERO, asi que el +1% que reportaba la vara vieja era convexidad de x/y, no la casa achicando margen. Tres reparos en pie: 63 de las 96 lineas no se movieron (el numero lo generan 33), el 'cierre' puede no ser el cierre porque el cron corre 09:00 y 15:00 y los partidos arrancan hasta las 21:00, y el ROI sigue en +7.37% **±21.33** — no dice nada hasta ~300 apuestas. Por metrica, remates da ROI -22.39% ±13.49 y al_arco +87.50% ±67.41: puede que el eje Jugadores sean dos ejes y uno reste. Ver TRASPASO §22 y §22bis | A mano, cada tanto — y de nuevo cuando haya más muestra |
...
| `medir_roi.py` | La regla de valor de la app contra **PLATA**, liga por liga: ROI, drawdown y Sharpe walk-forward. Reemplaza al script ad-hoc de Node que midió el único ROI que existía (arg, −3.27% ±6.19) y que no había quedado en el repo. **Re-medido el 2026-09-03 con el arnés corregido** (§37): hasta entonces calculaba λ crudo mientras producción aplica `corregir_escala` y `encoger_diferencia` y usa el rho de cada liga — o sea que medía un modelo y se publicaba otro. Los números nuevos: arg −6.96% ±7.46, bra −4.99% ±7.74, eng +2.21% ±9.53, fra −3.04% ±9.83, spa −6.95% ±8.87 — **ninguno significativo**, y arg/bra eran los dos únicos que salían del ruido antes. **`LIGAS_SIN_VALOR` NO se cambió**: la evidencia de daño se debilitó pero no apareció evidencia a favor, los dos puntos estimados siguen negativos, y `barrido_valor.py` rehecho sigue sin encontrar una sola ventana positiva. Pasale `liga=` a `evaluar()` o vuelve a medir el modelo crudo. Ver §37 y §38 | A mano, cada tanto |
...
| `medir_margen_props.py` | Cuánto margen cobra Bet365 en la escalera de jugador — el paso 0 del simulador de combinadas. La escalera es de un solo lado, así que el margen sale de comparar la implícita del precio contra la frecuencia real. Ojo con el pool: **el 30% de los jugadores que la casa cotiza no terminan siendo titulares**, y apostar la escalera entera pierde 53% en buena parte por eso. Solo titulares, el sobreprecio medio es **+9.0 pp** contra nuestra ventaja medida de +1.15 pp de CLV: la casa cobra ocho veces lo que ganamos, y una combinada multiplica el margen en vez de dividirlo. Reparo que no se puede resolver: el "real" sale de los remates de ESPN y Bet365 liquida con otra fuente. El CLV de §22bis no depende de esto y sigue en pie; los ROI de medir_props.py sí quedan en duda. Ver TRASPASO §31 | A mano |
| `medir_metamodelo.py` | ¿Podemos predecir CUÁNDO nos equivocamos? No qué pasa en el partido, sino cuánto creerle a nuestro propio número. Medido el 2026-09-03 sobre 23.790 partidos con train/test y una variable **placebo** en la misma tabla: **ninguna variable propia predice nuestro error relativo al mercado** — descanso, congestión, jornada, historia y volatilidad están todas en el orden de magnitud del placebo y la mayoría cambia de signo entre train y test. Se probó contra el error absoluto y por liga, por si la resta del mercado lo borrara o el efecto viviera solo donde el mercado es blando: tampoco. Lo único que predice es cuánto discrepamos del cierre (+0.1319, monótono en cinco quintiles), que es de mercado y solo sugiere copiar — y dice algo más duro: nuestro desacuerdo **no tiene contenido informativo a ninguna magnitud**. **Lo que sí salió**: contra el error ABSOLUTO, volatilidad (−0.245) y λ total (−0.148) son grandes y estables. No es ventaja —un cruce desparejo es más fácil para todos— pero es una medida honesta de incertidumbre del partido que la app no tiene. Ver TRASPASO §30 | A mano, cada tanto |
| `medir_bandas.py` | ¿Hay ventaja en las cuotas de 4 a 10? Sale de una pregunta de Lucas: si un contexto puede volver menos improbable lo improbable, ahí habría valor. Medido el 2026-09-03 sobre **23.870 partidos** de cinco ligas, walk-forward, con train/test temporal: **no**. `real − mercado` es negativo en las tres bandas largas —el mercado paga de menos, no de más— y apostar solo donde le ganamos al mercado da −7.38% ±2.82 en test, o sea que cuando discrepamos ahí discrepamos para el lado equivocado. El indicio que lo motivó (`medir_calibracion`, +8.2 puntos) eran 85 partidos sobre 12 mercados correlacionados y no sobrevive. **Lo que sí quedó**: agrupado por NUESTRA p el modelo está bien calibrado, y agrupado por la cuota está +3 a +5 en las largas — las dos juntas solo pueden pasar si nuestro error está correlacionado con el precio, o sea que no estamos descalibrados sino **menos informados**. Por eso ninguna recalibración funciona: se probaron la potencia global (óptimo k=0.90, aplanar) y la isotónica (+0.00006 de Brier en test). Ojo con el rho: el arnés mide con 0.05 y ninguna liga lo usa. Ver TRASPASO §29 | A mano, cada tanto |
...
| `probar_odds_api.py` | Si conviene cambiar DraftKings —la única casa que expone ESPN— por Pinnacle. Prueba de The Odds API. Necesita `ODDS_API_KEY`. **Ojo con la ilusión de que acá hay plata**: el ROI de `medir_roi.py` YA está medido a cierre de Pinnacle, así que el −7% (era −9% antes de §37) no es margen tirado. Bajar el margen no devuelve puntos que no se estaban perdiendo; lo que sí hace es que el número publicado sea optimista respecto de lo que paga la app. Ver TRASPASO §20 | A mano |
...
| `calibrar_senal.py` | El umbral de las cuatro dimensiones de volumen de `desarrollo.senal`. Existe porque hasta el 2026-09-03 no había ninguno: `generador` tenía su 50% y las otras cuatro quedaban a criterio, y en el primer smoke test eso se rompió solo — se declaró `null` una señal a +16% de la media de liga y se afirmó otra a +11%, en la misma corrida. **Se calibra contra historial DISJUNTO a propósito**: 20.897 partidos de seis ligas europeas (2015/16 a 2025/26), walk-forward con train/test temporal, porque un umbral elegido sobre la muestra que después se evalúa no es un umbral. Medido: faltas 10%/15%/6 acierta 71.2% contra una tasa base de 54.1%; remates igual umbral, 63.6% contra 51.2%; córners pide 20%/20%/6 y da 65.7% contra 52.0%. **`tarjetas` no supera su tasa base en NINGÚN umbral de la grilla (máximo 1.8 ee) y por eso no se afirma nunca.** El umbral lo aplica `expediente.py` en `senal_base` y la skill copia el fallo — la aritmética no es trabajo de criterio. Ver TRASPASO §34. `python calibrar_senal.py` | A mano, si alguien propone mover el umbral |
| `potencia_senal.py` | Cuánta muestra hace falta para validar `desarrollo.senal`. Aritmética sobre lo ya medido, no mide nada nuevo. **faltas: 64 señales / 433 partidos / ~17 semanas 🟢. remates: 125 / 1214 / ~47 semanas 🟢. córners: 102 / 5369 / 4 años 🔴** — córners queda afuera por COBERTURA (emite 1 de cada 53 partidos), no por tamaño de efecto. Y **la capa 3 no es cuestión de muestra**: desde §34 la señal es una función determinista del mismo `estadisticas.json` que genera su baseline, así que el aporte incremental es cero por construcción. Ver TRASPASO §39 | A mano |
| `medir_senal.py` | Las tres capas de `desarrollo.senal`: si la lectura afirma algo concreto, si se puede verificar, y si aporta sobre el pronóstico numérico de esa misma métrica. El baseline **no es λ** — λ no predice córners y cualquier cosa le ganaría. Ver TRASPASO §33 y §34 | A mano, cada tanto |
