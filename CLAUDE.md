...
| `medir_props.py` | El eje Jugadores contra **PLATA**, al precio real de Bet365 — la primera vez que se mide dinero acá, cuando `medir_jugadores.py` solo medía calibración. Usa la escalera de precios entera y las varias fotos por partido que `props_jugadores.json` venía acumulando. Cruza nombres **solo por igualdad exacta**: los que no cruzan son en su mayoría otros jugadores (Clever Ferreira contra Pablo Ferreira), y un cruce difuso los funde. Trae su propia vara: la **deriva** de los escalones que NO se apuestan, sin la cual un CLV positivo puede ser la casa achicando el margen sobre la hora. Trae ademas dos controles que el error estandar no da: **dejar un partido afuera por vez** (las apuestas de un mismo partido no son independientes) y **cuantas lineas quedaron clavadas** (ahi el CLV es cero por construccion). **Medido de nuevo el 2026-09-02, con 25 partidos y 89 apuestas: los dos controles que en agosto lo desarmaron ahora lo sostienen.** CLV +3.32% ±0.86, y contra la deriva de los 3214 escalones que NO se eligen (+0.99% ±0.29) elegimos mejor por **+2.33% ±0.91, 2.6 errores estandar**; dejando un partido afuera por vez queda entre +2.55% y +3.55%, asi que ninguno lo carga solo. La señal **crecio con la muestra en vez de desaparecer**, que es lo contrario de lo que hace el ruido. Tres reparos que siguen en pie: 62 de las 89 lineas no se movieron (ahi el CLV es cero por construccion, o sea que el numero lo generan 27), el 'cierre' puede no ser el cierre porque el cron corre 09:00 y 15:00 y los partidos arrancan hasta las 21:00, y el ROI sigue en +9.98% **±21.12** — no dice nada hasta ~300 apuestas. **Es el primer lead del proyecto que sobrevive a sus propios controles.** Ver TRASPASO §22 | A mano, cada tanto — y de nuevo cuando haya más muestra |
